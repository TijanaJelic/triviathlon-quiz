...
const randomNum = (maxNum) => (
  Math.floor(Math.random() * Math.floor(maxNum))
)
  ...
  return ( 
    <div className='questions-wrapper'>
      <div className='questions-container'>
        ...
        <h2>{(questions.length && currentQuestion !== questions.length) && questions[currentQuestion].question}</h2>
        <div className='answers-and-countdown'>
          <div className='answers-container'>
            {(questions.length && currentQuestion !== questions.length) && [ questions[currentQuestion].correctAnswer, ...questions[currentQuestion].incorrectAnswers].map((answer) => (
              <button className='answer' key={answer} onClick={(e)=> {
                isCorrect(e)
                handleClickAnswer(e)
              }}
              >{answer}</button>
            ...
            <Countdown date={Date.now() + 100000} renderer={timerRenderer} overtime={true} />
        </div>
      </div>
    </div>
  )
